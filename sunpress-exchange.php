...
function print_sunpress_exchange_options_form() {
		...
		echo "<option value='gbp'>Great British Pounds (GBP) - &pound;</option>"; 
		echo "<option value='eur'>Euro (EUR) - &euro;</option>"; 
		echo "<option value='usd'>United States Dollar (USD) - $</option>"; 
		echo "</select></p>";
		echo "<p><strong>Important:The last currency you used was <em>".strtoupper($default_currency)."</em>. You need to select a new currency, regardless of your previous selection.</strong></p>";
                    ...
                    foreach ($rss->items as $item) {
                        if ($counter < 6) {
                            ...
                            $rsstitle = $item['title'];
														...
														$rssdescription = str_replace("&", "&amp;",$rssdescription);
														$rssdescription = str_replace("from only", "<strong>from only",$rssdescription);
                            $rssdescription = str_replace("&amp;pound;".$rssprice."pp",$currency.number_format($newprice,2)."pp</strong>",$rssdescription);
